...
| PRE-03 | `ligne_porte_la_version_qui_la_produite` | ligne sans `methode.version`, sans `logiciel.version`, ou avec une version mal formée | Refus | Règle dure de l'ADR 0000 §6 : sans ces champs, la politique de version est décorative et une preuve publiée devient non interprétable. La version du **contrat**, elle, n'est plus portée par la ligne depuis le contrat `0.6.0` — elle décrit le format et vit dans le manifeste (PRE-15, EXP-12) |
...
| PRE-15 [C] | `contrat_absent_de_la_ligne_de_preuve` | ligne à laquelle on rajoute `contrat` ; les cinq lignes du §2.6 ; la liste close `CLES` | Refus du producteur, nommant la clé ; `contrat` absent de `CLES` et des cinq lignes de référence | `contrat` figurait dans chaque ligne sans entrer dans la clé de déduplication du §3 : **toute** bascule de version produisait 34 lignes de même `id` et de contenu différent, I8 arrêtait le pipeline, et le registre en ajout seul devait être réécrit — I15 enfreint. C'est arrivé en `0.4.0` puis en `0.5.0` : structurel, pas accidentel. La version du contrat décrit le **format**, pas la **mesure**. Sans ce test, le champ revient : il est plausible, et rien d'autre que la liste close ne l'interdit |
| PRE-16 [C] | `ligne_annonce_le_majeur_deux` | la constante `SCHEMA` ; une ligne portant `contrepoint/preuve/1` ; les cinq lignes du §2.6 ; les deux schémas formels | `SCHEMA` vaut `contrepoint/preuve/2` ; la ligne `/1` est refusée par I1 et le producteur ne peut pas l'écrire — `rendre` repose `schema` depuis la constante ; les cinq lignes annoncent `/2` ; `preuve-2.schema.json` porte le `const` `/2` et `preuve-1.schema.json` reste figé sur `/1` | Retirer `contrat` de la ligne est la ligne « champ supprimé, renommé, resémantisé » du §5, qui tarife un `schema` **majeur**. L'exception « aucune ligne publiée, donc aucun lecteur à protéger », invoquée en `0.2.0` et en `0.3.0`, s'est éteinte le 2026-08-28 avec la publication du site — le §5 annonçait lui-même cette extinction. Sans ce test le majeur se défait en une ligne : remettre `/1` dans `SCHEMA` laisse passer toute la suite, la valeur n'étant comparée à rien, et republie sous un majeur déjà publié une forme qui n'est plus la sienne. Le contrôle sur `preuve-1.schema.json` tient l'autre faute : le rebasculer sur `/2` laisserait sans schéma le lecteur d'un artefact récupéré avant la bascule |
...
| EXP-09 [C] | `tout rect de fond porte un remplissage declare, hors selecteur alterne` | `web/src/styles.css` | Une règle de base donne un `fill` à `.fond`, hors `:nth-child` | Un `<rect>` SVG sans `fill` est peint en **noir** par le navigateur. Seule la règle `:nth-child(odd)` existait : un système sur deux sortait en pavé noir, texte illisible. Invisible en thème sombre — le papier y est déjà presque noir — et flagrant en thème clair. **Constaté en ligne le 2026-08-28**, sur la page publiée, par une rastérisation. Ni les 29 tests de contrat ni le rendu figé ne pouvaient le voir : le premier ne lit que du JSON, le second n'embarque aucune feuille de style |
| EXP-10 [C] | `les lignes d un panneau sont croissantes en abscisse` | instantané **volontairement inversé** | Chaque panneau gradué range ses lignes par la valeur qu'il dessine | Support de l'amendement GV-07. Le panneau des experts rendait dans l'ordre de `bandes[]`, qui est un ordre de **votes** : le tracé sortait 0,82 · 3,45 · 6,60 · 7,73 · 8,82 · 2,30 · 1,73 · 6,27. L'entrée du test est inversée à dessein — sur les fixtures livrées, déjà croissantes, le test survivait au retrait du tri. Ce qui reste interdit : trier par effectif, par dispersion ou par état de mesure |
| EXP-11 [C] | `composition des groupes publiee sans effectif, et dite partielle` | instantané construit sur le registre réel ; registre dont une adhésion est close ; artefact altéré d'un effectif | La bande d'un GROUPE nomme les partis que le registre retient — GDR nomme le PCF, ECOS nomme les Écologistes et le PCF ; la clé est absente d'une bande de parti, d'une coalition et d'un groupe sans parti retenu ; aucun chiffre n'y figure ; le rendu écrit le mot « extrait » | Le site ne montrait que le nom du groupe : quelqu'un a cherché les communistes et les écologistes sans les trouver, alors qu'ils siègent à GDR et à ECOS. Deux fautes symétriques sont possibles, et ce test tient les deux. Publier la liste sans dire qu'elle est un EXTRAIT serait faux par omission — GDR n'y porte que le PCF quand ses membres déclarent aussi sept partis ultramarins hors périmètre. Publier un effectif serait pire : « 9 membres en cours déclarent PCF » n'existe que dans le champ `remarque`, en prose, et un nombre tiré d'une phrase est fabriqué, pas mesuré |
| EXP-12 [C] | `contrat_porte_par_le_format_jamais_par_la_mesure` | manifeste, instantané et éclats construits sur les lignes de fixture | Le manifeste et l'instantané portent `contrat` ; **aucune** ligne d'éclat ni du registre ne le porte | Versant artefact de PRE-15. Retirer le champ des deux côtés à la fois aurait été la faute symétrique : la version du format doit rester lisible quelque part, et sa place est l'artefact qui décrit le format, pas la ligne qui porte une mesure |
...
| **12** | Le registre de preuves | PRE-01 → PRE-16 | Le JSONL en ajout seul, rejouable, idempotent. Le contrat des briques 1-3 existe |
| **13** | L'export et le front | EXP-01 → EXP-12 | Le graphe de la roadmap v0.6, sur fixtures |
